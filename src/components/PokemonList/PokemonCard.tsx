...
const PokemonCard = ({ data: pokemonData, types }: PokemonCardProps) => {
  ...
  const [getTypes, { data: responseType }] = useLazyQuery<{
  ...
  const typelist = types || responseType?.pokemon.types;
  ...
  const type1 = typelist?.[0]?.type?.name || "unknown";
  const type2 = typelist?.[1]?.type?.name || type1 || "unknown";
